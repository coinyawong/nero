...
if [ ${ck} -eq 1 ]
then
../tezos/tezos-client transfer ${reward} from ledger_coinyawong_ed to ${account} --fee 0 > payout.out
echo $today $name "님" $cycle "사이클 보상완료 : ../tezos-client transfer ${reward} from ledger_coinyawong_ed to ${account} --fee 0" >> payout.out
else
